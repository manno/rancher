...
func run(ctx context.Context) error {
	...
	if transport != nil {
		rancherlog.Info("Testing connection to server using trusted certificate authorities", "server", server, "caFileLocation", caFileLocation)
		...
		if err != nil {
			if strings.Contains(err.Error(), "x509:") {
				...
				if _, err := os.Stat(caFileLocation); err == nil {
					...
					if len(blocks) > 1 {
						rancherlog.Warn("Found multiple certificates at CA file location, should be one", "count", len(blocks), "caFileLocation", caFileLocation)
					}
					rancherlog.Info("Certificate details for CA file", "caFileLocation", caFileLocation)
					...
					for _, block := range blocks {
						...
						rancherlog.Info("Certificate info", "index", blockcount, "caFileLocation", caFileLocation)
						certinfo(cert)
