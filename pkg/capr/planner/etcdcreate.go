...
log.Warn("[planner] rkecluster: skipping etcd snapshot creation as cluster has not yet been initialized or bootstrapped", "namespace", controlPlane.Namespace, "name", controlPlane.Name)
return status, nil
}
...
if err != nil {
log.Error("[planner] rkecluster: error encountered while searching for init node during etcd snapshot creation", "namespace", controlPlane.Namespace, "name", controlPlane.Name, "error", err)
return status, err
}
if !found || joinServer == "" {
log.Warn("[planner] rkecluster: skipping etcd snapshot creation as cluster does not have an init node", "namespace", controlPlane.Namespace, "name", controlPlane.Name)
return status, nil
}
