...
func calculateJoinURL(cp *rkev1.RKEControlPlane, entry *planEntry, plan *plan.Plan) string {
	...
	log.Debug("Determined join URL for machine",
		...
		"join_url", entries[scaled].Metadata.Annotations[capr.JoinURLAnnotation],
		"checksum", ck,
		"entries_count", uint32(len(entries)),
	...
	if joinURL == "" {
		...
		if joinURL == "" {
			...
			log.Info("Previous join server was not valid, using new join server",
				...
				"machine_namespace", entry.Machine.Namespace,
				"machine_name", entry.Machine.Name,
				"old_join_server", joinedTo,
				"new_join_server", joinURL)
			if joinURL == "" {
	...
	for _, r := range reconcilables {
		...
		if r.entry.Plan == nil {
			...
			log.Trace("Reconcile tier - initial plan for machine",
				...
				"machine_namespace", r.entry.Machine.Namespace,
				"machine_name", r.entry.Machine.Name,
				"plan", fmt.Sprintf("%+v", r.desiredPlan))
			...
			log.Trace("Reconcile tier - minor plan change for machine",
				...
				"machine_namespace", r.entry.Machine.Namespace,
				"machine_name", r.entry.Machine.Name,
				"old_plan", fmt.Sprintf("%+v", r.entry.Plan.Plan),
				"new_plan", fmt.Sprintf("%+v", r.desiredPlan))
			outOfSync = append(outOfSync, r.entry.Machine.Name)
			if err := p.store.UpdatePlan(r.entry, r.desiredPlan, r.joinedURL, maxFailures, failureThreshold); err != nil {
				return err
			}
