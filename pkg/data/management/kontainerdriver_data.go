...
func (c *driverCreator) deleteKontainerDriver(name, urlPrefix string) {
	...
	if err != nil {
		if !errors.IsNotFound(err) {
			log.Warn("Error getting kontainer driver for deletion", "name", name, "error", err)
		}
		return
	}
	...
	if driver.Spec.Active || !strings.HasPrefix(driver.Spec.URL, urlPrefix) {
		log.Info("Not deleting active or modified kontainer driver", "name", name)
		return
	}

	log.Info("Deleting kontainer driver", "name", name)
...
func (c *driverCreator) deleteBuiltInKontainerDriver(driverName string) {
	if err := c.drivers.Delete(driverName, &v1.DeleteOptions{}); err != nil && !errors.IsNotFound(err) {
		log.Warn("Error deleting built-in kontainer driver", "driverName", driverName, "error", err)
	}
}
